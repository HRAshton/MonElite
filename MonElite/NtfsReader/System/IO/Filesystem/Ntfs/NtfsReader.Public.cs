...
namespace System.IO.Filesystem.Ntfs
{
    ...
    public partial class NtfsReader
    {
        ...
        public List<INode> GetNodes(string rootPath)
        {
            ...
            //TODO use Parallel.Net to process this when it becomes available -- realized for MonElite
            UInt32 nodeCount = (UInt32)_nodes.Length;
            Parallel.For(0, nodeCount, i =>
            {
                if (_nodes[i].NameIndex != 0 && GetNodeFullNameCore((uint)i).StartsWith(rootPath, StringComparison.InvariantCultureIgnoreCase))
                    lock (nodes)
                    {
